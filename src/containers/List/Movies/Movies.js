...
class Movies extends Component {
    ...
    async componentDidMount() {        
        try{
            let movieLists = await axios.get(process.env.MOVIEDB_API_URL + "movie/now_playing?api_key=" + process.env.MOVIEDB_API_KEY + "&page=1");
    ...
    render(){
        const {movies, loading, cardWidth, textWidth} = this.state;
